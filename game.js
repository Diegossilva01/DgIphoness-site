(function(){
  ...
  // Caminho principal no tabuleiro 15x15, 52 posições.
  const PATH = [
  ...
  // Cada cor inicia em um ponto diferente no caminho comum.
  ...
  function newGame(players){
    return {
      ...
      pieces:players.flatMap((p,i)=>Array.from({length:4},(_,piece)=>({
        ...
        progress:-1 // -1 quintal, 0..51 caminho, 52..57 corredor, 58 finalizado
      }))),
  ...
  function move(game,playerId,pieceId){
    ...
    let captured=captureAt(game,piece);
    ...
    // Regra simplificada: tirar 6 concede nova jogada.
    if(rolled===6){ game.dice=null; game.rolled=false; game.serial++; game.log.unshift(`${player.name} joga novamente por ter tirado 6.`); }
    else nextTurn(game);
