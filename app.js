(() => {
    ...
    try{
      ...
      peer.on('error',err=>{
        ...
        if(err.type==='unavailable-id'){
          ...
          if(!isHost){ setBadge('procurando sala...',false); }
        } else {
  ...
  function handleHostMessage(conn,msg){
    ...
    if(msg.type==='join'){
      ...
      if(existing){
        ...
        existing.name=(msg.player.name||existing.name||'Jogador').slice(0,18);
  ...
  function joinRoom(name,code){
    myName=name||'Jogador';
    myId=sessionStorage.getItem('ludo-player-id')||playerId();
    sessionStorage.setItem('ludo-player-id',myId);
    roomCode=cleanCode(code); isHost=false;
    sessionStorage.setItem('ludo-room-code',roomCode);
    sessionStorage.setItem('ludo-player-name',myName);
  ...
  function connectToHost(initial=false){
    ...
    joinTimeout=setTimeout(()=>{
      ...
      els.joinError.textContent='A sala demorou para responder. Tentando reconectar...';
    ...
    conn.on('open',()=>{
      ...
      conn.send({type:'join',player:{id:myId,name:myName}});
    });
    conn.on('data',msg=>{
      ...
      if(msg.type==='joined' || msg.type==='state'){
        ...
        clearJoinTimeout();
        els.joinError.textContent=msg.message;
      }
    });
    conn.on('close',()=>{
      if(hostConnection!==conn) return;
  ...
  function scheduleGuestReconnect(){
    ...
    const delay=Math.min(900*Math.pow(1.6,reconnectAttempts++),7000);
    reconnectTimer=setTimeout(()=>{
      reconnectTimer=null;
